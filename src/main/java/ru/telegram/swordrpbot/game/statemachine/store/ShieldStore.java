...
public class ShieldStore extends State {
    @Override
    public BotApiMethod<?> enter(Message message) {
        String chatId = message.getChatId().toString();
        long userId = message.getFrom().getId();
        StringBuffer getMsg = new StringBuffer("Добро пожаловать в раздел Щиты");
        int i = 0;
        List<Shield> listShield = GoodsCache.Instance.getListShield();
        for(Shield it : listShield){
    ...
    public void update(String message, long userId) {
        ...
        if(message.matches("/buysh(.*)")) {
            String str = message.replaceAll("[^0-9]", "");
            int ind = Integer.valueOf(str);
            Shield shield = GoodsCache.Instance.getListShield().get(ind);
            try {
                user.getPlayer().setArmor(shield);
