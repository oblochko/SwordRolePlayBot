...
public class RangedStore extends State {
    @Override
    public BotApiMethod<?> enter(Message message) {
        String chatId = message.getChatId().toString();
        long userId = message.getFrom().getId();
        StringBuffer getMsg = new StringBuffer("Добро пожаловать в раздел Оружия дальнего боя");
        int i = 0;
        List<Ranged> listRanged = GoodsCache.Instance.getListRanged();
        System.out.println(listRanged.size());
        for(Ranged it : listRanged){
    ...
    public void update(String message, long userId) {
        ...
        if(message.matches("/buyrng(.*)")) {
            String str = message.replaceAll("[^0-9]", "");
            int ind = Integer.valueOf(str);
            Ranged ranged = GoodsCache.Instance.getListRanged().get(ind);
            try {
                user.getPlayer().setWeapon(ranged);
